...
#		$lh = $dbh->prepare("select distinct  l.name, l.tags->'name:de' as name_de,  l.tags->'name:fr' as name_fr, l.tags->'name:it' as name_it , l.tags->'name:rm' as name_rm, l.tags->'alt_name' as alt_name,l.tags->'official_name' as official_name,l.tags->'short_name' as short_name    from planet_osm_point l, planet_osm_polygon p where (l.place is not NULL) and p.osm_id = $polygon and ST_Intersects(l.way,p.way)");
        ...
        while(my $lref = $lh->fetchrow_hashref()) 
	{
		...
		{
			if (($GWR_names{$name_plz} eq  $osm_names{$name}) || ($GWR_names{$name_plz} eq  $osm_names_de{$name}) || ($GWR_names{$name_plz} eq  $osm_names_fr{$name}) || ($GWR_names{$name_plz} eq  $osm_names_it{$name}) || ($GWR_names{$name_plz} eq  $osm_names_rm{$name}) || ($GWR_names{$name_plz} eq  $osm_names_alt{$name}) || ($GWR_names{$name_plz} eq  $osm_names_official{$name}) || ($GWR_names{$name_plz} eq  $osm_names_left{$name}) || ($GWR_names{$name_plz} eq  $osm_names_right{$name})  || ($GWR_names{$name_plz} eq  $osm_names_short{$name}))
			{
				...
				elsif ($GWR_names{$name_plz} eq 'point')
				{
					$point_match++;
				}
				elsif ($GWR_names{$name_plz} eq 'area')
                                {
                                        $area_match++;
                                }
				else
...
#	print	$osm_road_count,"\t",$osm_area_count,"\t",$osm_point_count,"\t",$GWR_road_count,"\t",$GWR_area_count,"\t",$GWR_point_count,"\t",$GWR_none_count,"\t",$GWR_unknown_count,"\t",$exact_type_count,"\t",$exact_count,"\n";
	...
	print "<td align=\"right\">",$typos,"</td><td align=\"right\">",$osm_road_count,"</td><td align=\"right\">",$osm_area_count,"</td><td align=\"right\">",$osm_point_count,"</td><td class=\"grey\" align=\"right\">",$osm_road_count+$osm_area_count+$osm_point_count,"</td><td align=\"right\">",$GWR_road_count,"</td><td align=\"right\">",$GWR_area_count,"</td><td align=\"right\">",$GWR_point_count,"</td><td align=\"right\">",$GWR_none_count,"</td><td align=\"right\">",$GWR_unknown_count,"</td><td class=\"grey\" align=\"right\">",$GWR_road_count+$GWR_area_count+$GWR_point_count+$GWR_none_count+$GWR_unknown_count,"</td><td align=\"right\">",$exact_count,"</td><td align=\"right\">",$exact_type_count,"</td><td align=\"right\">",$unexact_road_match,"</td></tr>\n";
	...
	if (($GWR_none_count + $GWR_unknown_count) > 0)
	{
        ...
        my $sh = $dbh->prepare("update muni_name_stats_old set match_overall=$p_overall,match_road=$p_road,match_point=$p_point,match_area=$p_area,match_other=$p_other where muni_ref=$muni_ref");
        my $r = $sh->execute();
        if (undef == $r) {
                print STDERR "Update for $muni_ref failed\n";
                $sh = $dbh->prepare("insert into muni_name_stats_old (muni_ref,match_overall,match_road,match_point,match_area,match_other) values($muni_ref,$p_overall,$p_road,$p_point,$p_area,$p_other)");
                $r = $sh->execute();
                if (undef == $r) {
